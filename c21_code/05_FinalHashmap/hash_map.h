...
namespace ProCpp {
	...
	template <typename HashMap>
	class const_hash_map_reverse_iterator
	{
		// hash_map 클래스는 const_hash_map_reverse_iterator의 모든 원소에 접근할 수 있어야 한다.
		friend HashMap;
	...
	public:
		...
		using pointer = value_type * ;
		using reference = value_type & ;
		using list_iterator_type = typename HashMap::ListType::const_reverse_iterator;

		// 양방향 반복자는 반드시 기본 생성자를 제공해야 한다.
		// 기본 생성자로 만든 반복자를 사용하는 방법은 특별히 정해진 것이 없으므로
		// 원하는 방식으로 초기화하면 된다.
		const_hash_map_reverse_iterator() = default;

		const_hash_map_reverse_iterator(size_t bucket, list_iterator_type listIt, const HashMap* hashmap);
		...
		const_hash_map_reverse_iterator<HashMap>& operator++();
		const_hash_map_reverse_iterator<HashMap> operator++(int);
		...
		// 멤버 함수를 다음과 같이 정의해도 된다.
		// 다양한 타입과 비교하는 기능을 지원하지 않기 때문이다.
		bool operator==(const const_hash_map_reverse_iterator<HashMap>& rhs) const;
		bool operator!=(const const_hash_map_reverse_iterator<HashMap>& rhs) const;
	...
	template <typename HashMap>
	class hash_map_reverse_iterator : public const_hash_map_reverse_iterator<HashMap>
	{
		// hash_map 클래스는 const_hash_map_iterator의 모든 원소에 접근할 수 있어야 한다.
		friend HashMap;

	public:
		using value_type = typename const_hash_map_reverse_iterator<HashMap>::value_type;
		using difference_type = ptrdiff_t;
		using iterator_category = std::bidirectional_iterator_tag;
		using pointer = value_type * ;
		using reference = value_type & ;
		...
		hash_map_reverse_iterator() = default;
		hash_map_reverse_iterator(size_t bucket, list_iterator_type listIt, HashMap* hashmap);

		value_type& operator*();
		value_type* operator->();

		hash_map_reverse_iterator<HashMap>& operator++();
		hash_map_reverse_iterator<HashMap> operator++(int);
